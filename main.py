...
class ScoreLabel(MDGridLayout):
    def __init__(self, right_side, **kwargs):
        ...
        super().__init__(cols=1, size_hint=[.4, .2], md_bg_color=[0, 0, 0, 1], pos_hint={'center': [pos_hint_x, .5]},
                         **kwargs)
...
class GameOver(MDRelativeLayout):
    def __init__(self, *args, **kwargs):
        super().__init__(md_bg_color=[0, 0, 0, 1], size_hint=[1, 1], pos_hint={'center': [.5, -.5]},
                         adaptive_height=True, *args, **kwargs)
        self.lbl = MDLabel(theme_text_color='Custom', text_color=[1, 1, 1, 1], font_style='H1', valign='center',
...
class NameInputs(MDGridLayout):
    def __init__(self, x2=False, *args, **kwargs):
        ...
        self.name1 = MDTextField(text=TEXTS.get(10), **default_args)
        self.add_widget(self.name1)
        if self.x2:
            self.name2 = MDTextField(text=TEXTS.get(11), **default_args)
...
class HistoryChart(Popup):
    ...
    def open(self, *args):
        ...
        for i in range(len(p1)):
            ...
            bg.add_widget(MDLabel(text=str(i), adaptive_width=True))
...
class SingleGame(MDRelativeLayout):
    def __init__(self, app, **kwargs):
        ...
        self.hist = HistoryChart(size_hint=[.8, .35], pos_hint={'center': [.5, .8]})
        self.timer = MDRelativeLayout(md_bg_color=[0, 0, 0, 1], size_hint=[.01, 0],
                                      pos_hint={'center_y': .5, 'center_x': .5})

        # Title
        if not hasattr(self, 'tittle'):
            self.tittle = MDLabel(text=TEXTS.get(1), font_style='H3', pos_hint={'center': [.5, .93]}, max_lines=1,
        ...
        if not hasattr(self, 'names_popup'):
            ...
            self.names_popup = Popup(title=TEXTS.get(3), content=names_grid, size_hint=[.5, 1], title_align='center',
    ...
    def build(self):
        ...
        self.add_widget(MDRectangleFlatButton(line_color=[0, 0, 0, 0], md_bg_color=[.7, 0.7, 0.7, 1], text=TEXTS.get(6),
        ...
        self.add_widget(
            MDRectangleFlatButton(line_color=[0, 0, 0, 0], md_bg_color=[.7, 0.7, 0.7, 1],
                                  text=TEXTS.get(7), size_hint=[.2, .1],
        ...
        self.add_widget(
            MDRectangleFlatButton(line_color=[0, 0, 0, 0], md_bg_color=[.7, 0.7, 0.7, 1], text=TEXTS.get(9),
    ...
    def change_names(self, team1, team2, *args):
        ...
        self.hist.update_names([*team1.get_names()], [*team2.get_names()])
...
class DoubleGame(SingleGame):
    def __init__(self, *args, **kwargs):
        ...
        self.team2 = Team(right_side=True, x2=x2, team_number=2, board=self,
                          pos_hint={'center': [.65, .6]})
        self.tittle = MDLabel(text=TEXTS.get(2), font_style='H3', pos_hint={'center': [.5, .93]}, max_lines=1,
                              halign='center', valign='center')
